...
function sortTable(column, sort_asc) {
    [...table_rows].sort((a, b) => {
        ...
        if (!isNaN(first_row) && !isNaN(second_row)) {
            ...
            // Nếu không phải là số, sử dụng localeCompare để so sánh
...
const delete_printer = () => {
    let printer_list = document.querySelectorAll('[id^="delete-printer-"]');
    printer_list.forEach(printer => {
        printer.addEventListener('click', () => {
            printer.parentNode.parentNode.remove();
        });
    });
}

const add_printer_form_open = () => {
    document.getElementById("add-printer-form").style.display = "";
    document.getElementById("main-screen").style.display = "none";
}

const add_printer_form_close = () => {
    document.getElementById("add-printer-form").style.display = "none";
    document.getElementById("main-screen").style.display = "";
}

const add_printer = () => {
    let table_body = document.getElementById("list-of-printer");
    let printer_id = document.getElementById("printer-id").value;
    let printer_name = document.getElementById("printer-name").value;
    let building_name = document.getElementById("building-name").value;
    htmls = `
            <tr class="hover-bg-gray" >
                <td>${printer_id}</td>
                <td>${printer_name}</td>
                <td>${building_name}</td>
                <td><input type="checkbox" class="btn-check" id="cbprinter${printer_id}" autocomplete="off" checked>
                <label class="btn btn-outline-primary" id="lbprinter${printer_id}" for="cbprinter${printer_id}">Bật</label>
                </td>
